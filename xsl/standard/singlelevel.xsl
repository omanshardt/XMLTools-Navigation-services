...
<xsl:output media-type="text/html" method="html" indent="yes" encoding="utf-8" omit-xml-declaration="yes" />
	...
	<xsl:template match="siteMap">
		<ul>
			<xsl:apply-templates select="descendant::siteMapNode[count(ancestor::*) = $level and (parent::siteMap or parent::siteMapNode/descendant-or-self::siteMapNode[@url=$upid])]" />
		</ul>
	</xsl:template>
